...
class POIIdentifier:
    ...
    POI_PROBABILITY: Dict[str, float] = {
        "IDM_SWEEP": 0.97,
        "EXTREME": 0.92,
        "DECISION": 0.87,
        "BOS_SWEEP": 0.82,
    ...
    # -----------------------
    def classify_poi_type(self,
                          ob: Dict[str, Any],
                          all_obs: List[Dict[str, Any]],
                          choch_level: Optional[float] = None,
                          idm_level: Optional[float] = None,
                          bos_level: Optional[float] = None,
                          price_tolerance: float = 1.0) -> str:
        """
        Guardeer Lecture 6 — Classify an OB into one of 5 valid POI types.

        Priority order (highest to lowest):
          IDM_SWEEP   -> OB whose range contains the inducement sweep level
          EXTREME     -> Last OB before CHoCH (furthest from current price)
          DECISION    -> First OB from current price (closest)
          BOS_SWEEP   -> OB at a BOS level
          CHOCH_SWEEP -> OB at a CHoCH level
          TRAP        -> Everything else (middle OBs) — REJECTED

        Args:
            ob:              The OB dict to classify (must have price_top, price_bottom, bar_index)
            all_obs:         Full list of OBs of the same type (for relative positioning)
            choch_level:     Price level of the most recent CHoCH
            idm_level:       Price level of the inducement sweep
            bos_level:       Price level of the BOS
            price_tolerance: +-tolerance in price units for level proximity
                             Default 1.0 = $1.00 for XAUUSD Gold

        Returns:
            "IDM_SWEEP" | "EXTREME" | "DECISION" | "BOS_SWEEP" | "CHOCH_SWEEP" | "TRAP"
        """
        ob_high = float(ob.get("price_top", 0))
        ob_low  = float(ob.get("price_bottom", 0))

        # Rule 1: IDM_SWEEP — OB range contains the inducement level
        if idm_level is not None:
            if ob_low - price_tolerance <= idm_level <= ob_high + price_tolerance:
                return "IDM_SWEEP"

        # Rule 2: EXTREME — OB with smallest bar_index among valid OBs
        # (formed earliest = furthest from current price = last before CHoCH)
        ob_type   = ob.get("type", "BULLISH")
        same_type = [o for o in all_obs if o.get("type") == ob_type and o.get("is_valid_basic")]
        if same_type:
            extreme_ob = min(same_type, key=lambda x: x.get("bar_index", 0))
            if ob.get("bar_index") == extreme_ob.get("bar_index"):
                return "EXTREME"

        # Rule 3: DECISION — OB with largest bar_index (most recent = closest to price)
        if same_type:
            decision_ob = max(same_type, key=lambda x: x.get("bar_index", 0))
            if ob.get("bar_index") == decision_ob.get("bar_index"):
                return "DECISION"

        # Rule 4: BOS_SWEEP — OB at a BOS level
        if bos_level is not None:
            if ob_low - price_tolerance <= bos_level <= ob_high + price_tolerance:
                return "BOS_SWEEP"

        # Rule 5: CHOCH_SWEEP — OB at a CHoCH level
        if choch_level is not None:
            if ob_low - price_tolerance <= choch_level <= ob_high + price_tolerance:
                return "CHOCH_SWEEP"

        # Everything else = middle OB = TRAP (never trade)
        return "TRAP"

    # -----------------------
    # get_active_pois() — query method for NarrativeAnalyzer.build_narrative()
    # -----------------------
    def get_active_pois(self,
                        current_price: Optional[float] = None,
                        max_distance: float = 50.0,
                        tradeable_only: bool = False) -> List[Dict[str, Any]]:
        """
        Return active (non-TRAP, non-INVALID, non-WEAK) POIs, optionally filtered
        by proximity to current price and tradeability.

        Used by NarrativeAnalyzer.build_narrative() to answer B2:
        "Has price rebalanced an FVG/OB?"

        Args:
            current_price:  Filter by price proximity (None = no distance filter)
            max_distance:   Max price distance from current_price to include
                            Default 50.0 = $50 range for XAUUSD Gold
            tradeable_only: If True, only return POIs where permission_to_trade=True

        Returns:
            List of OB dicts sorted by proximity to current_price (closest first).
            Each dict includes:
              "tapped"      bool  — True if current_price is inside the OB zone right now
              "distance"    float — price distance from current_price to OB midpoint
              "probability" float — Guardeer win-probability from POI_PROBABILITY map
        """
        all_finalized = (
            self.order_blocks.get("bullish", []) +
            self.order_blocks.get("bearish", [])
        )

        active = []
        for ob in all_finalized:
            block_class = ob.get("block_class", "")

            # Skip non-tradeable block classes
            if block_class in ("TRAP", "INVALID", "WEAK", "POTENTIAL_OB"):
                continue

            # Optionally filter by permission gate
            if tradeable_only and not ob.get("permission_to_trade", False):
                continue
            ...
            if current_price is not None:
                tapped    = ob_low <= current_price <= ob_high
                mid_price = (ob_high + ob_low) / 2.0
                distance  = abs(current_price - mid_price)

                if distance > max_distance:
                    continue

            prob = self.POI_PROBABILITY.get(block_class, 0.0)
        ...
        # Sort: closest first when price known, otherwise most recent first
        if current_price is not None:
            active.sort(key=lambda x: x["distance"])
        else:
            active.sort(key=lambda x: x.get("bar_index", 0), reverse=True)
